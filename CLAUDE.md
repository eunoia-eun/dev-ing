...
- **배치(발령) 이력**: `domain/employee/Assignment.ts`(employeeId·department·jobTitle·startDate·endDate?·note, from-to). `AssignmentRepository`(`assignments` 키). `AssignmentService` — `getTimeline`(이력 없으면 입사일 기준 최초 배치 1건 lazy 생성·persist), **`changeAssignment`**(이전 배치 종료[전날]→새 배치 시작→임직원 현재 부서·업무 갱신→진행 중 노출 종료[전날]→새 부서 매핑 유해인자를 `suggestions`로 반환). 부서·업무 변경은 이 흐름을 거쳐야 노출이 함께 정리된다. UI는 `EmployeeProfilePage`의 **통합 "배치 · 유해물질 노출 이력" 카드** — 배치를 `group-row` 헤더(기간·부서·업무·현재 뱃지)로, 그 아래에 해당 기간 노출(노출 시작일이 배치 기간에 포함되면 소속, 미매칭은 맨 아래 '배치 미매칭' 묶음)을 묶어 표시. 카드 액션 "발령/배치 변경"(2단계 모달: 입력 → 새 노출 제안 체크 등록, `hazard.addExposure`로 startDate=변경일) + "＋ 노출 등록", 배치 헤더마다 "＋ 이 배치에 노출"(해당 배치 시작일로 기본 채움). 배치 헤더는 **클릭 시 접기/펼치기**(기본: 모두 펼침 — 부서·노출 변경 이력을 한눈에 확인; 사용자가 누른 id만 `Set`에 기억해 접음). **현재 배치엔 부서별 유해인자 매핑(카탈로그)의 '표준 유해인자' 칩 행**을 함께 표시 — 등록됨(✓)/미등록(＋칩·일괄 등록)으로 `hazard.getDepartmentHazards()` + `addExposure`를 조합해 그 자리에서 노출 등록(부서별 유해인자↔배치↔노출 통합). 합성 루트에서 assignment·exposure·deptHazard repo를 주입.
...
- **인증·권한·임직원 포털**: `domain/auth/Account.ts`(`Account{id·employeeId(관리자는 null)·employeeNumber(로그인 사번)·role: 'manager'|'employee'·hashedPassword}`). 포트 `IAccountRepository`(findByEmployeeId/findByEmployeeNumber/findManagerAccount/save/list)·`IPasswordHasher`(hash/verify), 구현 `WebCryptoHasher`(SHA-256). `AuthService` — `initialize()`(관리자 계정이 없으면 사번 `admin`/초기 비밀번호 `admin1234`로 생성), `login(employeeNumber, password, role)`(관리자: 저장된 `hashedPassword` 검증; 임직원: 계정이 없으면 생년월일 8자리(YYYYMMDD)를 초기 비밀번호로 검증 후 자동가입[해시 저장], 있으면 검증), `changePassword(accountId, currentPassword, newPassword)`, `resetEmployeePassword(employeeId)`(관리자가 임직원 비밀번호를 생년월일로 초기화), `updateManagerNumber`, `listAccounts`. `AuthContext`(`useAuth()`)는 `auth.initialize()` 이후 `ready=true`, 세션은 localStorage(`whm:session`)에 저장/복원한다 — `session: {accountId, role, employeeId, employeeName, employeeNumber} | null`. 라우팅(`App.tsx`)은 `session`이 없으면 `LoginPage`만 노출, `role==='employee'`면 `ui/pages/employee/*`(`EmployeeLayout` 탭: 홈 `/`·내 유해인자 `/hazards`·건강정보 `/health`·건강프로그램 `/programs`), 관리자면 기존 보건관리자 화면. `AccountSettingsPage`(`/settings`, 모든 역할 공통)는 비밀번호 변경 + 임직원 본인 연락처(phone) 수정 + 관리자용 "임직원 계정 관리"(임직원별 계정 등록 여부 표시 + 비밀번호를 생년월일로 초기화하는 버튼). 합성 루트는 `LocalStorageAccountRepository`/`WebCryptoHasher`를 주입하고, 테스트 하네스는 `PlainTextHasher`(crypto.subtle 없이 동작) + 인메모리 `accounts` 저장소를 사용한다.
